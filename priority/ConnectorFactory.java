...
public class ConnectorFactory {
	public ConstraintConnector merger(String p1, String p2, String p3) {
		String merger = String.format("(%s" + RIGHTLEFTARROW + "(%s" + OR + "%s))" + AND +
										"(" + NOT + "(%s" + AND +"%s))" +
									 AND + "((" + NOT + "%s )" + IMPLIES + "((" + NOT + "%sk )" + AND + "%sk" + AND + "%sk)" +
									 		OR + "(%sc" + AND + "" + NOT + " %sk " + AND + " %sk)" +
									 		OR + "(%sc" + "" + AND + "" +" %sk" + AND + NOT + "%sk))"+
									 AND + "(" + NOT + " (%s" + AND + "%s))"
				, 
				flow(p3), flow(p1), flow(p2),
				flow(p1), flow(p2),
				flow(p3), p3, p1, p2,
				p3, p1, p2,
				p3, p1, p2,
	...
	public ConstraintConnector sync(String p1, String p2) {
		String sync = String.format("(%s" + RIGHTLEFTARROW + "%s)" + AND + NOT + "(%sc" + AND + "%sk)",
				flow(p1), flow(p2), p1, p2);
		return new ConstraintConnector(sync, p1, p2);
	}
/*
	public ConstraintConnector fullFifo(String p1, String p2) {
		String fullFifo = String.format(
										"(" + NOT + "%s)" + AND + "%s" + AND + "(%s" + IMPLIES + "("+ NOT +"%s))", 
										flow(p1), mem(p1, p2), flow(p2), nextMem(p1, p2)  
									);
		return new ConstraintConnector(fullFifo, p1, p2, mem(p1, p2), nextMem(p1, p2));
	}*/
		...
		if (full)
			fifo = String.format(
				...
				//	(full ? "%s" : "%s")
					...
					flow(p1),
					mem(p1, p2),
					flow(p2), nextMem(p1, p2)//,
				//	flow(p1), nextMem(p1, p2),
				//	flow(p2), mem(p1, p2, full), nextMem(p1, p2),
				//	mem(p1, p2, full), nextMem(p1, p2)
				);
			/*fifo = String.format(
								"(" + NOT + "(%s" + OR + "%s)" + IMPLIES + " (%s" + RIGHTLEFTARROW + "%s)) " + AND +
								"(%s" + IMPLIES + "(%s" + AND + "("+ NOT +"%s))) " + AND +
								"(%s" + IMPLIES + "(%s" + AND + "("+ NOT +"%s))) " + AND +
								(full ? "%s" : "%s")
								,
								flow(p1), flow(p2), mem(p1, p2, full), nextMem(p1, p2),
								flow(p1), nextMem(p1, p2), mem(p1, p2, full),
								flow(p2), mem(p1, p2, full), nextMem(p1, p2),
								mem(p1, p2, full), nextMem(p1, p2)
							);*/
		else
	...
	public ConstraintConnector syncDrain(String p1, String p2) {
		String syncDrain = String.format(
					"(" + NOT + "(%s" + AND + "%s))", 
					flow(p1), flow(p2)
					);
	...
	public ConstraintConnector prioritySync(String p1, String p2) {
		String prioritySync = String.format(
					"(%s" + RIGHTLEFTARROW + "%s)" + AND + NOT + "(%sc" + AND + "%sk)" + AND + "%sbullet" + AND + "%sbullet",
					flow(p1), flow(p2), p1, p2, p1, p2
					);
		return new ConstraintConnector(prioritySync, p1, p2);
	}
	
	public ConstraintConnector replicator(String c, String k1, String k2) {
		String replicator = String.format(
				"(%s" + RIGHTLEFTARROW + "%s)"+ AND + "(%s" + RIGHTLEFTARROW + "%s)",
				//+ NOT + "(%sc" + AND + "%sk)" + AND + "%sbullet" + AND + "%sbullet",
